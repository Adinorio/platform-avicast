    ...
    try:
        ...
        if user_count > 0:
            ...
            for user in recent_users:
                print(f"     - {user.employee_id} ({user.get_full_name()}) - {user.date_joined.strftime('%Y-%m-%d')}")
        ...
        if site_count > 0:
            ...
            for site in recent_sites:
                print(f"     - {site.name} ({site.site_type}) - {site.created_at.strftime('%Y-%m-%d')}")
        ...
        if census_count > 0:
            ...
            for census in recent_census:
                print(f"     - {census.site.name} - {census.observation_date} - {census.created_at.strftime('%Y-%m-%d')}")
        ...
        if species_count > 0:
            ...
            for species in recent_species:
                print(f"     - {species.species_name} (Count: {species.count}) - {species.created_at.strftime('%Y-%m-%d')}")
...
def check_recent_activity():
    ...
    try:
        ...
        from apps.locations.models import Site, CensusObservation
