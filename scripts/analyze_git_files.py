...
def analyze_repository(root_path):
    ...
    for category, stats in sorted(
        category_stats.items(), key=lambda x: x[1]["total_size_mb"], reverse=True
    ):
        print("20" f"{stats['count']:>5}")
    ...
    for file_info in all_files:
        ...
        try:
            result = (
                os.popen(f'git ls-files --error-unmatch "{relative_path}" 2>/dev/null')
                .read()
                .strip()
            )
            if result:
    ...
    if large_untracked:
        ...
        for file_info in large_untracked[:10]:
            print("6.1f" f"{file_info['category']}")
    ...
    if any(
        "runs" in f["path"] or "checkpoint" in f["path"] or "weight" in f["path"] for f in all_files
    ):
    ...
    # Large media files
    media_files = [
        f for f in all_files if f["category"] in ["VIDEO_FILES", "IMAGE_FILES"] and f["size_mb"] > 5
    ]
    if media_files:
    ...
    print(
        f"📊 Repository contains {len(all_files)} files totaling {sum(f['size_mb'] for f in all_files):.1f} MB"
    )
