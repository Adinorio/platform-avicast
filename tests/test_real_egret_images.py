        ...
        for image_path in test_images:
            ...
            if "detections" in results and results["detections"]:
                ...
                for i, detection in enumerate(detections):
                    ...
                    print(
                        f"  {i+1}. {class_name} (conf: {conf:.3f}) at [{bbox.get('x1',0):.1f}, {bbox.get('y1',0):.1f}, {bbox.get('x2',0):.1f}, {bbox.get('y2',0):.1f}]"
                    )
                    ...
                    if "decision" in detection:
                        ...
                        print(
                            f"     Decision: {decision.get('status', 'unknown')} - {decision.get('reason', 'no reason')}"
                        )
                    ...
                    if "stage2_decision" in detection:
                        ...
                        print(
                            f"     Stage 2: {stage2.get('status', 'unknown')} - {stage2.get('reason', 'no reason')}"
                        )
            else:
