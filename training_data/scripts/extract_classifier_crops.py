                ...
                for result in results:
                    ...
                    if boxes is not None:
                        for box in boxes:
                            ...
                            # Save original crop
                            self.save_crop(crop, class_name, split, f"{Path(image_path).stem}_orig_{class_counts[class_name]}")
                            class_counts[class_name] += 1

                            # Apply augmentations for minority classes
                            if class_name in ['Little Egret', 'Intermediate Egret'] and class_counts[class_name] < max_crops_per_class:
                                for aug_idx in range(3):  # 3 augmentations per original
                                    if class_counts[class_name] >= max_crops_per_class:
                                        break

                                    augmented = self.augment(image=crop)['image']
                                    self.save_crop(augmented, class_name, split, f"{Path(image_path).stem}_aug{aug_idx}_{class_counts[class_name]}")
                                    class_counts[class_name] += 1
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='classifier_training_data',
                       help='Output directory for crops')
    parser.add_argument('--max-crops', type=int, default=2000,
                       help='Maximum crops per class')
