...
class WeatherForecastAdmin(admin.ModelAdmin):
    list_display = ('site', 'forecast_date', 'forecast_time', 'temperature', 'weather_condition', 'field_work_score', 'field_work_recommendation', 'api_source')
    ...
    fieldsets = (
        ('Location & Timing', {
            'fields': ('site', 'forecast_date', 'forecast_time')
        }),
        ('Weather Data', {
            'fields': ('temperature', 'humidity', 'wind_speed', 'wind_direction', 'precipitation', 'pressure', 'weather_condition', 'visibility')
        }),
        ('Tide Information', {
            'fields': ('tide_condition', 'tide_height'),
            'classes': ('collapse',)
        }),
        ('API Information', {
            'fields': ('api_source', 'api_response_data'),
            'classes': ('collapse',)
        }),
        ('Field Work Analysis', {
            'fields': ('field_work_score', 'field_work_recommendation'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
...
class FieldWorkScheduleAdmin(admin.ModelAdmin):
    list_display = ('title', 'site', 'planned_date', 'planned_time_range', 'status', 'assigned_personnel_count', 'weather_optimization', 'created_by')
    list_filter = ('status', 'planned_date', 'site', 'created_by__role')
    search_fields = ('title', 'description', 'site__name', 'created_by__employee_id')
    readonly_fields = ('created_by', 'created_at', 'updated_at', 'weather_score', 'weather_recommendation')
    ordering = ('-planned_date', '-planned_start_time')
    
    fieldsets = (
        ('Schedule Information', {
            'fields': ('title', 'description', 'site', 'status')
        }),
        ('Timing', {
            'fields': ('planned_date', 'planned_start_time', 'planned_end_time', 'actual_start_time', 'actual_end_time')
        }),
        ('Personnel', {
            'fields': ('assigned_personnel', 'supervisor')
        }),
        ('Weather Optimization', {
            'fields': ('weather_score', 'weather_recommendation'),
            'classes': ('collapse',)
        }),
        ('Notes & Results', {
...
class WeatherAlertAdmin(admin.ModelAdmin):
    list_display = ('title', 'alert_type', 'severity', 'valid_period', 'affected_sites_count', 'is_active', 'issued_at')
    ...
    fieldsets = (
        ...
        ('Timing', {
            'fields': ('valid_from', 'valid_until', 'issued_at')
        }),
        ('Affected Areas', {
            'fields': ('sites',)
        }),
        ('Source Information', {
            'fields': ('source', 'external_id', 'metadata'),
            'classes': ('collapse',)
        }),
        ('Status', {
            'fields': ('is_active',),
