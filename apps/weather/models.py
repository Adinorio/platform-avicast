...
class WeatherForecast(models.Model):
    ...
    temperature = models.DecimalField(
        max_digits=4, decimal_places=1, help_text="Temperature in Celsius"
    )
    humidity = models.IntegerField(
        validators=[MinValueValidator(0), MaxValueValidator(100)], help_text="Humidity percentage"
    )
    wind_speed = models.DecimalField(max_digits=4, decimal_places=1, help_text="Wind speed in km/h")
    wind_direction = models.CharField(
        max_length=10, help_text="Wind direction (N, S, E, W, NE, etc.)"
    )
    ...
    tide_condition = models.CharField(
        max_length=20, choices=TideCondition.choices, null=True, blank=True
    )
    tide_height = models.DecimalField(
        max_digits=4, decimal_places=2, null=True, blank=True, help_text="Tide height in meters"
    )
...
class FieldWorkSchedule(models.Model):
    ...
    supervisor = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="supervised_field_work"
    )

    # Status and optimization
    status = models.CharField(
        max_length=20, choices=ScheduleStatus.choices, default=ScheduleStatus.PLANNED
    )
    weather_score = models.IntegerField(
        null=True, blank=True, help_text="Weather suitability score (0-100)"
    )
    weather_recommendation = models.CharField(
        max_length=20, choices=FieldWorkRecommendation.choices, null=True, blank=True
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="created_field_work"
    )
