...
class RealEvaluationService:
    ...
    def _resolve_model_path(self, model_name: str) -> tuple[str | None, str | None]:
    ...
    def run_inference(
        self, model_name: str, image_path: str, conf_threshold: float = 0.25
    ) -> list[Detection]:
    ...
    def calculate_metrics(self, matches: list, unmatched_preds: list, unmatched_gts: list) -> dict:
    ...
    def evaluate_image(
        ...
        logger.info(
            f"Starting evaluation of {os.path.basename(image_path)} with {len(models)} models"
        )
        ...
        for i, model_name in enumerate(models):
            ...
            logger.info(
                f"Model {model_name} found {len(preds)} predictions in {inference_time:.3f}s"
            )
            ...
            logger.info(
                f"Model {model_name} metrics: Precision={model_metrics['precision']:.3f}, Recall={model_metrics['recall']:.3f}, F1={model_metrics['f1_score']:.3f}"
            )
