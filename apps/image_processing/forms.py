...
from .models import AIModel, ImageUpload, ProcessingBatch
        ...
        if image_file:
            ...
            if image_file.size > IMAGE_CONFIG["MAX_FILE_SIZE_BYTES"]:
                ...
                raise forms.ValidationError(
                    f"Image file size must be less than {IMAGE_CONFIG['MAX_FILE_SIZE_MB']}MB."
                )
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Enter batch name..."}
            ),
...
class ModelSelectionForm(forms.Form):
    ...
    ai_model = forms.ChoiceField(
        ...
        widget=forms.Select(attrs={"class": "form-select", "id": "ai-model-select"}),
    ...
    confidence_threshold = forms.FloatField(
        ...
        widget=forms.NumberInput(
            attrs={"class": "form-control", "step": 0.05, "min": 0.1, "max": 1.0}
        ),
...
class StorageCleanupForm(forms.Form):
    ...
    days = forms.IntegerField(
        ...
        widget=forms.NumberInput(attrs={"class": "form-control", "min": "1", "max": "3650"}),
    )
