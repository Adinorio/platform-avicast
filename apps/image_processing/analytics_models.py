...
class ModelEvaluationRun(models.Model):
    ...
    iou_threshold = models.DecimalField(max_digits=3, decimal_places=2, default=0.50, help_text="IoU threshold for TP/FP classification")
    confidence_threshold = models.DecimalField(max_digits=3, decimal_places=2, default=0.25, help_text="Minimum confidence for detections")
    
    # Filters applied
    models_evaluated = models.JSONField(default=list, help_text="List of model variants included in evaluation")
    date_range_start = models.DateTimeField(null=True, blank=True, help_text="Start date for filtered data")
    date_range_end = models.DateTimeField(null=True, blank=True, help_text="End date for filtered data")
    ...
    overall_map_50 = models.DecimalField(max_digits=6, decimal_places=4, null=True, blank=True, help_text="mAP@0.5")
    overall_map_50_95 = models.DecimalField(max_digits=6, decimal_places=4, null=True, blank=True, help_text="mAP@0.5:0.95")
    ...
    processing_duration = models.DurationField(null=True, blank=True, help_text="Time taken to complete evaluation")
...
class ModelPerformanceMetrics(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    evaluation_run = models.ForeignKey(ModelEvaluationRun, on_delete=models.CASCADE, related_name='model_metrics')
    
    # Model identification
    model_name = models.CharField(max_length=50, help_text="Model variant name (e.g., yolov8l)")
    model_version = models.CharField(max_length=100, blank=True, help_text="Specific model version or checkpoint")
    ...
    # mAP metrics
    map_50 = models.DecimalField(max_digits=6, decimal_places=4, null=True, blank=True, help_text="mAP@0.5")
    map_50_95 = models.DecimalField(max_digits=6, decimal_places=4, null=True, blank=True, help_text="mAP@0.5:0.95")
    
    # Performance characteristics
    avg_inference_time_ms = models.DecimalField(max_digits=8, decimal_places=2, null=True, blank=True)
    avg_confidence_score = models.DecimalField(max_digits=5, decimal_places=4, null=True, blank=True)
    
    # Model parameters and size
    model_parameters_millions = models.DecimalField(max_digits=8, decimal_places=2, null=True, blank=True)
...
class SpeciesPerformanceMetrics(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    model_metrics = models.ForeignKey(ModelPerformanceMetrics, on_delete=models.CASCADE, related_name='species_metrics')
    ...
    recall = models.DecimalField(max_digits=6, decimal_places=4, null=True, blank=True)
    f1_score = models.DecimalField(max_digits=6, decimal_places=4, null=True, blank=True)
    average_precision = models.DecimalField(max_digits=6, decimal_places=4, null=True, blank=True, help_text="AP@0.5 for this species")
...
class ConfusionMatrixEntry(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    evaluation_run = models.ForeignKey(ModelEvaluationRun, on_delete=models.CASCADE, related_name='confusion_matrix')
    
    # Matrix coordinates
    actual_species = models.CharField(max_length=100, help_text="Ground truth species")
    predicted_species = models.CharField(max_length=100, help_text="Predicted species (or 'background' for FN)")
    
    # Count and percentage
    count = models.IntegerField(default=0, help_text="Number of instances with this actual/predicted combination")
    percentage = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, help_text="Percentage of total predictions")
    
    # Additional context
    avg_confidence = models.DecimalField(max_digits=5, decimal_places=4, null=True, blank=True, help_text="Average confidence for this combination")
    avg_iou = models.DecimalField(max_digits=5, decimal_places=4, null=True, blank=True, help_text="Average IoU for this combination")
...
class ImageEvaluationResult(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    evaluation_run = models.ForeignKey(ModelEvaluationRun, on_delete=models.CASCADE, related_name='image_results')
    ...
    avg_iou = models.DecimalField(max_digits=5, decimal_places=4, null=True, blank=True, help_text="Average IoU of matched boxes")
