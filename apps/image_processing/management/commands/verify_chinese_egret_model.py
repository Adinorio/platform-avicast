...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("🔍 Verifying Chinese Egret Model Integration"))
        ...
        try:
            ...
            status = (
                self.style.SUCCESS("✅ YES") if chinese_egret_loaded else self.style.ERROR("❌ NO")
            )
            ...
            if chinese_egret_loaded:
                ...
                self.stdout.write(
                    f"🏆 Chinese Egret Specialist status: {chinese_egret_info.get('status', 'Unknown')}"
                )
                ...
                if options["switch_to_chinese_egret"]:
                    ...
                    if switch_result:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS("✅ Successfully switched to Chinese Egret model!")
                        )
                        self.stdout.write(
                            f"🎯 New current model: {updated_info['current_version']}"
                        )
                        self.stdout.write(
                            f"📊 New performance: {updated_info['current_performance']}"
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR("❌ Failed to switch to Chinese Egret model")
                        )

                # Show how to use the model
                self.stdout.write("\n💡 HOW TO USE THE CHINESE EGRET MODEL:")
                self.stdout.write(
                    "   1. In your Django app, go to Model Selection (/image_processing/models/)"
                )
                self.stdout.write('   2. Select "🏆 Chinese Egret Specialist (99.46% mAP)"')
                self.stdout.write(
                    "   3. Upload Chinese Egret images for ultra-high accuracy detection"
                )
                self.stdout.write(
                    '   4. Check logs for "🏆 Loading Chinese Egret Specialist model" messages'
                )
