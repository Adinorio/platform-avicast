...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        stuck_uploads = ImageUpload.objects.filter(
            upload_status=ImageUpload.UploadStatus.UPLOADING, uploaded_at__lt=cutoff_time
        )
        ...
        self.stdout.write(f"Found {count} stuck upload(s) older than {older_than_hours} hour(s):")
        ...
        if dry_run:
            ...
            for upload in stuck_uploads:
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f"Failed to delete {upload.pk}: {e}"))

            self.stdout.write(
                self.style.SUCCESS(f"Successfully cleaned up {deleted_count} stuck upload(s)!")
            )
