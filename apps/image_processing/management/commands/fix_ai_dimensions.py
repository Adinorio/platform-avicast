...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Starting AI dimensions fix..."))
        ...
        if total_count == 0:
            self.stdout.write(self.style.WARNING("No processing results found"))
            return
        ...
        for i, result in enumerate(processing_results, 1):
            try:
                ...
                if not result.bounding_box or "ai_dimensions" not in result.bounding_box:
                    self.stdout.write("  No bounding box or ai_dimensions found, skipping")
                    continue

                # Get the image file
                image_upload = result.image_upload
                if not image_upload.image_file:
                    self.stdout.write("  No image file found, skipping")
                    continue
                ...
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f"  Error reading image: {e}"))
                    error_count += 1
                    continue
                ...
                needs_fixing = stored_dimensions != actual_dimensions or force

                if needs_fixing:
                    if dry_run:
                        self.stdout.write(
                            self.style.WARNING(
                                f"  Would fix: {stored_dimensions} -> {actual_dimensions}"
                            )
                        )
                    else:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"  Fixed: {stored_dimensions} -> {actual_dimensions}"
                            )
                        )
                        fixed_count += 1
                else:
                    self.stdout.write("  Dimensions already correct")

            except Exception as e:
                self.stdout.write(self.style.ERROR(f"  Error processing result {result.id}: {e}"))
                error_count += 1
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.SUCCESS("Run without --dry-run to apply the fixes"))
        else:
            self.stdout.write(
            ...
            if fixed_count > 0:
                self.stdout.write(
                    self.style.SUCCESS(
                        "All existing images should now display correct bounding boxes!"
                    )
                )
