...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force cleanup even if not near storage limits'
        )
        parser.add_argument(
            '--optimize-images',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'🧹 Starting storage cleanup (dry_run={dry_run})')
        )
        ...
        if hasattr(storage_backend, 'get_storage_usage'):
            ...
            if usage_info['usage_percentage'] > 80 or force:  # Use 80% as threshold
                ...
                self.stdout.write(
                    self.style.SUCCESS("✅ Storage usage is within limits, use --force to cleanup anyway")
                )
                return
    ...
    def _optimize_uncompressed_images(self, dry_run=False):
        ...
        for image_obj in uncompressed:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"❌ Failed to optimize {image_obj.id}: {e}")
                )
