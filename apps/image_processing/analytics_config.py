...
ANALYTICS_CONFIG = {
    ...
    'CHART_COLORS': [
        ...
        '#43e97b', '#38f9d7', '#e0c3fc', '#fa709a', '#a8edea'
    ],
...
def get_available_models():
    ...
    try:
        models = list(ImageProcessingResult.objects.values_list(
            'ai_model', flat=True
        ).distinct())
...
def get_target_species():
    ...
    try:
        species = list(ImageProcessingResult.objects.filter(
            detected_species__isnull=False
        ).values_list('detected_species', flat=True).distinct())
...
def get_admin_roles():
    ...
    try:
        ...
        admin_roles = [choice[0] for choice in User.ROLE_CHOICES
                      if 'ADMIN' in choice[0]]
...
def get_helpful_tips(status, progress=None):
    ...
    if status == 'PENDING':
        ...
        tips.append(UI_TEXT['HELPFUL_TIPS']['PROCESSING'][0].format(
            step=getattr(progress, 'current_step', 'Unknown')
        ))
        tips.append(UI_TEXT['HELPFUL_TIPS']['PROCESSING'][1].format(
