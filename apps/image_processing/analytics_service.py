...
class ModelAnalyticsService:
    ...
    def __init__(self):
        ...
        self.species_mapping = {0: "Chinese Egret", 1: "Whiskered Tern", 2: "Great Knot"}
    ...
    def load_ground_truth_annotations(self, image_filename: str) -> list[dict]:
        ...
        label_filename = (
            image_filename.replace(".jpg", ".txt").replace(".png", ".txt").replace(".jpeg", ".txt")
        )
                ...
                for line in f:
                    ...
                    if len(parts) >= 5:
                        ...
                        ground_truth.append(
                            {
                                "class_id": class_id,
                                "species": self.species_mapping.get(
                                    class_id, f"Bird_Class_{class_id}"
                                ),
                                "bbox": [x, y, w, h],
    ...
    def extract_predictions_from_result(
        self, processing_result: ImageProcessingResult
    ) -> list[dict]:
        """
        ...
        ai_dimensions = (
            tuple(ai_dimensions_raw) if isinstance(ai_dimensions_raw, list) else ai_dimensions_raw
        )
    ...
    def match_predictions_to_ground_truth(
        self, predictions: list[dict], ground_truth: list[dict], iou_threshold: float = 0.5
        ...
        try:
            ...
            for model_name, model_results in results_by_model.items():
                ...
                for result in model_results:
                    image_filename = (
                        result.image_upload.original_filename if result.image_upload else "unknown"
                    )
                    ...
                    if species_filter:
                        ground_truth = [
                            gt for gt in ground_truth if gt["species"] in species_filter
                        ]
                        predictions = [
                            pred for pred in predictions if pred["species"] in species_filter
                        ]
                    ...
                    ImageEvaluationResult.objects.create(
                        ...
                        if (image_tp + image_fp) > 0
                        else 0,
                        image_recall=image_tp / (image_tp + image_fn)
                        if (image_tp + image_fn) > 0
                        else 0,
                        inference_time_ms=float(result.inference_time) * 1000
                        if result.inference_time
                        else None,
                    )
                ...
                if all_predictions and all_ground_truth:
                    ...
                    for class_id, species_name in self.species_mapping.items():
                        if not species_filter or species_name in species_filter:
                            ...
                            species_preds = [
                                p for p in all_predictions if p["class_id"] == class_id
                            ]
                            species_gt = [
                                gt for gt in all_ground_truth if gt["class_id"] == class_id
                            ]

                            species_matches, species_fp, species_fn = (
                            ...
                            if species_tp + species_fp_count > 0:
                                species_metrics.precision = Decimal(species_tp) / Decimal(
                                    species_tp + species_fp_count
                                )
                            if species_tp + species_fn_count > 0:
                                species_metrics.recall = Decimal(species_tp) / Decimal(
                                    species_tp + species_fn_count
                                )
                            if (
                                species_metrics.precision
                                and species_metrics.recall
                                and (species_metrics.precision + species_metrics.recall) > 0
                            ):
                                species_metrics.f1_score = (
                                    2
                                    * (species_metrics.precision * species_metrics.recall)
                                    / (species_metrics.precision + species_metrics.recall)
                                )
                ...
                if model_inference_times:
                    metrics.avg_inference_time_ms = Decimal(
                        str(np.mean(model_inference_times) * 1000)
                    )
            ...
            if overall_tp + overall_fp > 0:
                evaluation_run.overall_precision = Decimal(overall_tp) / Decimal(
                    overall_tp + overall_fp
                )
            if overall_tp + overall_fn > 0:
                evaluation_run.overall_recall = Decimal(overall_tp) / Decimal(
                    overall_tp + overall_fn
                )
            if (
                evaluation_run.overall_precision
                and evaluation_run.overall_recall
                and (evaluation_run.overall_precision + evaluation_run.overall_recall) > 0
            ):
                evaluation_run.overall_f1_score = (
                    2
                    * (evaluation_run.overall_precision * evaluation_run.overall_recall)
                    / (evaluation_run.overall_precision + evaluation_run.overall_recall)
                )
    ...
    def run_kfold_evaluation(
        ...
        try:
            ...
            if date_range:
                start_date, end_date = date_range
                results_qs = results_qs.filter(created_at__gte=start_date, created_at__lte=end_date)
            ...
            for res in results:
                image_filename = (
                    res.image_upload.original_filename if res.image_upload else "unknown"
                )
            ...
            model_names = (
                ...
                else [m for m in by_model.keys() if m in model_filters]
            )
            ...
            for model_name in model_names:
                ...
                for fi in range(max(1, k)):
                    ...
                    for it in fold_items:
                        ...
                        all_gt.extend(gt)
                        matches, unmatched_preds, unmatched_gt = (
                            self.match_predictions_to_ground_truth(preds, gt, float(iou_threshold))
                        )
                    ...
                    for class_id, species_name in self.species_mapping.items():
                        if species_filter and species_name not in species_filter:
                            continue
                        ap = self.calculate_average_precision(
                            all_preds, all_gt, class_id, float(iou_threshold)
                        )
                        aps.append(ap)
                    if aps:
                    ...
                    all_model_maps[model_name].append(
                        float(metrics.map_50) if metrics.map_50 else 0.0
                    )
                    all_model_precisions[model_name].append(
                        float(metrics.precision) if metrics.precision else 0.0
                    )
                    all_model_recalls[model_name].append(
                        float(metrics.recall) if metrics.recall else 0.0
                    )
                    all_model_f1s[model_name].append(
                        float(metrics.f1_score) if metrics.f1_score else 0.0
                    )
    ...
    def summarize_reviewed_results(
        self, species_filter: list[str] = None, model_filters: list[str] = None
    ) -> dict:
        """
        ...
        if species_filter:
            qs = qs.filter(
                Q(overridden_species__in=species_filter) | Q(detected_species__in=species_filter)
            )
        if model_filters:
    ...
    def summarize_model_performance_from_reviews(self, model_filters: list[str] = None) -> dict:
        ...
        for model, stats in by_model.items():
            ...
            stats["species_accuracy"] = (
                (stats["correctly_identified_species"] / reviewed_count)
                if reviewed_count > 0
                else 0
            )

            # Of all AI detections, what portion was accepted as-is by humans?
            stats["practical_precision"] = (
                (stats["detections_approved_by_human"] / ai_detections) if ai_detections > 0 else 0
            )
    ...
    def calculate_statistical_significance(
        self, model_metrics_list, metric_name="f1_score", confidence_level=0.95
    ):
        """
            ...
            for j in range(i + 1, len(model_names)):
                ...
                if len(data1) > 1 and len(data2) > 1:
                    ...
                    pairwise_tests[f"{model1} vs {model2}"] = {
                        ...
                        "effect_size": abs(np.mean(data1) - np.mean(data2))
            ...
            model_data.update(
                {
                    ...
                    if (metrics.true_positives + metrics.false_positives + metrics.false_negatives)
                    > 0
                    else 0,
    ...
    def _calculate_specificity(self, metrics):
        ...
        total_possible_detections = (
            metrics.images_processed * 10
        )  # Assume avg 10 possible detection areas per image
        true_negatives = (
            total_possible_detections
            - metrics.true_positives
            - metrics.false_positives
    ...
    def _analyze_errors(self, evaluation_run):
        ...
        for result in image_results:
            ...
            for fp in result.unmatched_predictions:
                ...
                error_analysis["common_false_positives"][error_type] = (
                    error_analysis["common_false_positives"].get(error_type, 0) + 1
                )

            # Analyze false negatives
            for fn in result.unmatched_ground_truth:
                error_type = f"False Negative: {fn.get('species', 'unknown')}"
                error_analysis["common_false_negatives"][error_type] = (
                    error_analysis["common_false_negatives"].get(error_type, 0) + 1
                )
