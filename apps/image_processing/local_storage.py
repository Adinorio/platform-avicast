...
class LocalStorageManager:
    ...
    def get_storage_usage(self) -> dict:
        ...
        active_usage = active_files.aggregate(total_size=Sum("file_size"), count=Count("id"))

        archived_usage = archived_files.aggregate(total_size=Sum("file_size"), count=Count("id"))
    ...
    def archive_old_files(self, days_old: int = 30) -> dict:
        ...
        old_files = ImageUpload.objects.filter(
            uploaded_at__lt=cutoff_date, storage_tier__in=["HOT", "WARM"]
        )
            ...
            try:
                if file_obj.image_file and os.path.exists(file_obj.image_file.path):
                    ...
                    archive_dir = (
                        self.archive_path
                        / "bird_images"
                        / str(file_obj.uploaded_at.year)
                        / str(file_obj.uploaded_at.month)
                    )
    ...
    def _is_ssd_storage() -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["wmic", "diskdrive", "get", "MediaType"], capture_output=True, text=True
            )
