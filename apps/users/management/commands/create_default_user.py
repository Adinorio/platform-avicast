...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if User.objects.filter(username='010101').exists():
            self.stdout.write(self.style.SUCCESS('✅ Default superadmin user "010101" already exists.'))
            return
        ...
        if User.objects.count() == 0:
            ...
            try:
                ...
                self.stdout.write(self.style.SUCCESS('✅ Default superadmin user created successfully!'))
                self.stdout.write(f'   Username: {user.username}')
                self.stdout.write(f'   Employee ID: {user.employee_id}')
                self.stdout.write(f'   Role: {user.role}')
                self.stdout.write(f'   Password: avicast123')
