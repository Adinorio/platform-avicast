...
class CustomUserAdmin(BaseUserAdmin):
    ...
    fieldsets = (
        ...
        (
            "Account Settings",
            {"fields": ("role", "account_status", "is_active", "is_staff", "is_superuser")},
        ),
        (
            "Password Management",
            {"fields": ("password_changed", "password_changed_date", "password_change_required")},
        ),
...
class DataRequestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Request Details", {"fields": ("request_data", "related_species", "related_site")}),
        ("Approval Process", {"fields": ("approved_by", "approved_at", "admin_notes")}),
        ("Field Worker Notes", {"fields": ("field_worker_notes",)}),
    ...
    def get_queryset(self, request):
        ...
        if request.user.role == User.Role.SUPERADMIN:
            ...
            return qs.filter(
                status__in=[
                    DataRequest.RequestStatus.PENDING,
                    DataRequest.RequestStatus.APPROVED,
                    DataRequest.RequestStatus.REJECTED,
                ]
            )
        else:
